...
int main() {
    ...
    const char* kernelSource = "__kernel void onesComplement(__global const int* input, __global int* output) {\n"
    ...
    cl_kernel kernel = clCreateKernel(program, "onesComplement", NULL);
    ...
    // Print the one's complement values
    printf("One's complement values:\n");
    for (int i = 0; i < N; ++i) {
        printf("%d (Binary) -> %d (One's Complement)\n", inputArray[i], outputArray[i]);
    }
